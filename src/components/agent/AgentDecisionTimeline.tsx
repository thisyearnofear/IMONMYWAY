  ...
  return (
    <div className="h-full p-4">
      <div className="flex items-center justify-between mb-3">
        ...
        <span className="text-xs font-mono text-white/60">{decisions.length} decisions</span>
      </div>
          ...
          {decisions.slice(0, 10).map((event, i) => {
            ...
            return (
              <motion.div
                ...
                initial={{ opacity: 0, x: -20, boxShadow: 'inset 0 0 0 0 rgba(234,196,108,0)' }}
                animate={{
                  opacity: 1,
                  x: 0,
                  boxShadow: isSettlement
                    ? ['inset 0 0 20px 0 rgba(234,196,108,0.3)', 'inset 0 0 0 0 rgba(234,196,108,0)']
                    : 'inset 0 0 0 0 rgba(234,196,108,0)',
                }}
                exit={{ opacity: 0 }}
                transition={{
                  delay: i * 0.05,
                ...
                className={`flex items-start gap-2 py-1.5 border-b border-white/5 last:border-0 rounded-sm ${
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <span className={`text-xs font-bold ${isSettlement ? (isSuccess ? 'text-gold-500' : 'text-red-400') : 'text-white/90'}`}>
                      {event.type === 'decision'
