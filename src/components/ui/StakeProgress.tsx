...
export function StakeProgress({
  ...
  useEffect(() => {
    ...
    const simulatedDistance =
      timeProgress * estimatedDistance * (0.8 + Math.random() * 0.4);
  ...
  const getProgressColor = () => {
    ...
    if (timeLeft < 300000 && progress < 0.7)
      return "from-red-500 to-orange-500"; // 5 min left, behind
    if (timeProgress > 0.7 && progress < 0.7)
      return "from-orange-500 to-yellow-500"; // Behind schedule
        ...
        <div>
          ...
          <div className="font-medium text-gray-900">
            {estimatedDistance - distanceCovered < 1000
              ...
              : `${((estimatedDistance - distanceCovered) / 1000).toFixed(
                  1
                )}km`}
          </div>
        </div>
