...
export default function PlanPage() {
  ...
  const planRoute = useCallback(async () => {
    if (!startAddress.trim() || !endAddress.trim()) {
      addToast({ message: 'Please enter both start and end addresses', type: 'warning' })
      return
    }
    ...
    try {
      ...
      if (!startCoords || !endCoords) {
        addToast({ message: 'Could not find one or more addresses', type: 'error' })
        return
      }
  ...
  const shareRoute = () => {
    ...
    const shareText = `Check out my planned run: ${formatDistance(planData.distance)} in ${formatTime(planData.eta)} at ${selectedPace} min/mile pace.`
    ...
    if (typeof window !== 'undefined' && navigator.share) {
      ...
      addToast({ message: 'Route details copied to clipboard', type: 'success' })
    }
  }
    ...
    return (
      <div className="min-h-screen bg-gray-50">
        ...
        <div className="fixed inset-0">
          <MapContainer
            className="h-full"
            center={planData ? planData.startCoords : [40.7128, -74.0060]}
            onMapReady={(map) => {
      ...
      <main className="container mx-auto px-4 py-8 max-w-4xl">
        <div className="text-center mb-6">
          ...
          <p className="text-gray-600">
            Enter start and end locations to calculate distance and estimated time
          </p>
        </div>
              ...
              <div className="mt-6 p-4 bg-gradient-to-br from-blue-50 to-purple-50 rounded-xl border border-blue-100">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <div className={`px-2 py-1 rounded-full text-xs font-medium ${
                    ...
                    {planData.confidence >= 80 ? 'High' : planData.confidence >= 65 ? 'Med' : 'Low'} {planData.confidence}% confident
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-4 mb-4">
                  <div className="text-center">
                    <div className="text-lg font-bold text-blue-600">{formatDistance(planData.distance)}</div>
                    <div className="text-xs text-gray-600">Distance</div>
                  </div>
                  <div className="text-center">
                    <div className="text-lg font-bold text-purple-600">{formatTime(planData.eta)}</div>
        ...
        <div className="mt-8 bg-white rounded-lg shadow-md p-6">
          ...
          <div className="grid md:grid-cols-3 gap-4 text-sm text-gray-600">
            <div>
              <h3 className="font-medium text-gray-900 mb-1">Accurate Addresses</h3>
              <p>Use specific landmarks or full addresses for better route planning</p>
            </div>
            <div>
            ...
            <div>
              <h3 className="font-medium text-gray-900 mb-1">Share Your Plan</h3>
              <p>Let others know your route and expected arrival time for safety</p>
            </div>
          </div>
        </div>
      </main>
